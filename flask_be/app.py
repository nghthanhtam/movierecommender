...
class User(Resource):
    def get(self, user_id):
        user = mongo.db.user.find_one({"_id": ObjectId(user_id) },{ "hashed_password" : 0})
        if user is None:
            return response('',404)
        else:
...
class UserList(Resource):
    ...
    def post(self):
        ...
        if username and email and password :
            ...
            mongo.db.user.insert_one({'username': username, 'email': email, 'hashed_password': hashed_password })
...
class UserLogin(Resource):
    def post(self):
        ...
        if check_password_hash(new_user['hashed_password'], password):
            encoded_jwt = jwt.encode({'username': new_user['username'], 'email': new_user['email'], 'exp': 24 * 3600  }, SECRET_KEY, algorithm='HS256')
